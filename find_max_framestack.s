...
main:	push {r0, r1, lr}@ salva contenuto registri
	...
	push {r0,r1} @carico nello stack i valori aggiornati
...
find_max: 
   push {r11, lr} @ salvo il valore lr e fp nello stack
   mov r11, sp @ aggiorno il valore di fp
   mov r2, #1 @contatore
   ldr r1, [r11,#12]	@ carica la lunghezza dell'array in r1
   ldr r3, [r11, #8] @carico il valore di r0.
   
loop:
	LDR R4, [R0, R2, LSL #2]  @carico in r4 il valore dell array di indice corrispondente
	cmp r3,r4 @confronto i due numeri
	movlt r3, r4 @se r3 è minore cambio valore e metto il valore di r4
	add r2, r2, #1 @aumento il contatore
	cmp r2, r1 @controllo nel contatore
	bne loop @finchè il contatore non è uguale rifaccio il loop
	mov r0, r3 @metto il numero maggiore in r0
	mov sp, r11
	pop {r11, lr}
	pop {r0, r1} @ripristino i dati
	mov pc, lr
